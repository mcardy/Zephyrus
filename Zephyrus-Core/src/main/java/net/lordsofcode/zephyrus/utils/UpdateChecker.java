 ...
 * 
 * @author Gravity Modified by Minnymin3 to work with Zephyrus file naming
 * @version 2.0
 */
...
public class UpdateChecker {
	...
	private UpdateChecker.UpdateResult result = UpdateChecker.UpdateResult.SUCCESS; // Used
																					// for
																					// determining
																					// the
																					// outcome
	...
	private boolean read() {
		try {
			...
			this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1))
					.get(UpdateChecker.VERSION_VALUE);
			...
			if (UpdateChecker.this.url != null) {
				...
				if (UpdateChecker.this.read()) {
					if (UpdateChecker.this.versionCheck(UpdateChecker.this.versionName)) {
						if ((UpdateChecker.this.versionLink != null)
								&& (UpdateChecker.this.type != UpdateType.NO_DOWNLOAD)) {
							...
							UpdateChecker.this.saveFile(new File(UpdateChecker.this.plugin.getDataFolder().getParent(),
									UpdateChecker.this.updateFolder), name, UpdateChecker.this.versionLink);
						} else {
